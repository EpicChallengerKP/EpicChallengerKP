# Hi, I'm Kamalpreet Singh Nafray

UE5 Game Designer / Developer | AI Enthusiast | Storyteller

I’m a curious and passionate creator building games, tools, and interactive experiences in Unreal Engine 5, with a focus on emotional depth, experimentation, and meaningful player experiences.



## What I’m Working On

- **Psychological Horror Game (University Project)**  
  Creative Director on a UE5 team project exploring sanity systems, distorted choices, and psychological storytelling.

- **NovaRoads (UE5 Plugin)**  
  A personal R&D plugin for procedural road generation with Blueprint splines and non-destructive landscape deformation.

- **3D Dating Sim (Personal Project)**  
  An early-stage narrative-driven game about isolation, connection, and contrast through realistic character interactions.



## Tech I Use

- Unreal Engine 5  
- UE Blueprints, Python and sometimes C++
- Blender, ZBrush, Substance Painter
- Procedural systems, landscape tools, and post-processing



## Let’s Connect

Always happy to chat about Unreal Engine, game design, storytelling, or creative projects.

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kpsnafray/) [![Instagram](https://img.shields.io/badge/Instagram-FF0069?logo=instagram&logoColor=white)](https://www.instagram.com/epic_challenger_kp) [![X (Twitter)](https://img.shields.io/badge/X-000000?logo=X&logoColor=white)](https://x.com/kp_epic)

## Github Stats

  <p align="center"><a href="https://awesome-github-stats.azurewebsites.net/index.html??cardType=level&theme=github-dark&fontFamily=Advent%20Pro&preferLogin=true&borderRadius=24&Border=585858&Ring=71C3DD&Text=7BA0AAD0">    <img  alt="EpicChallengerKP's GitHub Stats" src="https://awesome-github-stats.azurewebsites.net/user-stats/EpicChallengerKP?cardType=level&theme=github-dark&fontFamily=Advent%20Pro&preferLogin=true&borderRadius=24&Border=585858&Ring=71C3DD&Text=7BA0AAD0" />  </a> </p>

## 🍻 Cheers 

Thanks for stopping by!
